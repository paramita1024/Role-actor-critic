...
def run_episode(config):
    ...
    role_model_dir = config.role_model_path.replace('env_id',config.env_id)
    ...
    if True:#with open('maac_vs_maac.py') as f:
        ...
        role_model = RoleAttentionSAC.init_from_save(role_model_dir, load_critic=True)
        role_model.prep_rollouts(device='cpu')
        role_model.config.n_rollout_threads=config.n_rollout_threads
        ...
        while True:
            ...
            # print('torch agent actions',type(torch_agent_actions))
            # for ac in torch_agent_actions:
            #     print('ac',type(ac.data))

            print('episode step',episode_step)
            agent_actions = [ac.data.numpy() for ac in torch_agent_actions]
            # return 
            role_agent_actions = role_model.step(torch_obs, explore=True)
            # print('role agent actions',type(role_agent_actions))
            # for ac in role_agent_actions:
            #     print('ac',type(ac.data))

            role_agent_actions = [ac.data.numpy() for ac in role_agent_actions[0]]
            # return 

            if config.method=='maac_vs_role':
                action_n = agent_actions[:2] + role_agent_actions[2:]
            ...
            if done or terminal:
                ...
                if config.trajectory:
                    ...
                    # end=time.time()
                    # print('Time ta?ken: ', end-start, ' seconds')
                    # start=time.time()
                    trajectory=init_traj(config.env_id)
