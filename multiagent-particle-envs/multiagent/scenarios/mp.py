...
class Scenario(BaseScenario):
    def make_world(self, ep_length=50,ep_pos=None):
        ...
        type_of_pennies = 2
        world.resource_types = list(range(type_of_pennies))
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.silent = True
            agent.ghost = True
            agent.holding = np.array([0.0, 0.0])
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.type = world.resource_types[i%type_of_pennies]
    ...
    def calculate_reward(self, world):
        team_1 = np.array([0.0, 0.0])
        team_2 = np.array([0.0, 0.0])
    ...
    def team_1_reward(self, agent, world):
        ...
        if self.t == self.ep_length:
            # print('yes')
            tmp = self.calculate_reward(world)
            reward += tmp
            # print('team 2 reward', tmp)
        return (reward,agent.holding)
