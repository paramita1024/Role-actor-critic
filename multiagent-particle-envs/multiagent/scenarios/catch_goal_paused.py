...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        #print('you are outside, not inside')
    ...
    def agent_reward(self, agent, world):
        stat=0
        # if someone has already touched the landmark
        if world.first_land:
            return (0.0,stat)
        # the minimum distance to the goal
        agent_dist = [-np.sqrt(np.sum(np.square(agent.state.p_pos - l.state.p_pos))) for l in world.landmarks]
        ...
        own_team = [-np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for l in world.landmarks for a in world.agents if not a.adversary]
        opp_team = [-np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for l in world.landmarks for a in world.agents if a.adversary]
    ...
    def adversary_reward(self, agent, world):
        ...
        # calculate whether any team has reached the flag or not
        own_team = [-np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for l in world.landmarks for a in world.agents if a.adversary]
        opp_team = [-np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for l in world.landmarks for a in world.agents if not a.adversary]
