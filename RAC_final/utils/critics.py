...
class AttentionCritic(nn.Module):
    ...
    """
    def __init__(self, sa_sizes, role_dim, hidden_dim=32, norm_in=True, attend_heads=1):
        """
        Inputs:
        ...
        for sdim, adim in sa_sizes:
            ...
            if norm_in:
                state_encoder.add_module('s_enc_bn', nn.BatchNorm1d(
                                            sdim + role_dim, affine=False))
            state_encoder.add_module('s_enc_fc1', nn.Linear(sdim + role_dim,
                                                            hidden_dim))
    ...
    def forward(self, inps, role, agents=None, return_q=True, return_all_q=False,
        ...
        true_states = [s for s, a in inps]
        states = [torch.cat((s, r), dim=1) for s, r in zip(true_states, role)]
        actions = [a for s, a in inps]
        inps = [torch.cat((s, a), dim=1) for s, a in zip(states, actions)]
